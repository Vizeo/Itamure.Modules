...
export class MoviesListComponent implements IVideoManager {
	...
	public async DeleteMediaItem() {
		if (this.EditingVideoMediaItem != null &&
			this.EditingVideoMediaItem.Id != null &&
			confirm("Delete media item " + this.EditingVideoMediaItem.Name + "?")) {
			await this.mediaService.RemoveMediaFileItem(this.EditingVideoMediaItem.Id);
			let index = this.MoviesVideoMediaItems!.indexOf(this.EditingVideoMediaItem.TargetObject);
